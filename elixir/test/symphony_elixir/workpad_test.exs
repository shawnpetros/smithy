  ...
  defmodule StubClient do
    ...
    defp replace_comment_body(comments_by_issue, comment_id, body) do
      ...
      |> Enum.map(fn {issue_id, comments} ->
        {issue_id, Enum.map(comments, &replace_comment(&1, comment_id, body))}
      end)
      |> Map.new()
    end

    defp replace_comment(%{id: comment_id} = comment, comment_id, body), do: %{comment | body: body}
    defp replace_comment(comment, _comment_id, _body), do: comment
