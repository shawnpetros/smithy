defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    assert config.tracker.active_states == ["Todo", "In Progress", "Adversarial Review", "Rework"]
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    down_started_ms = System.monotonic_time(:millisecond)
    send(pid, {:DOWN, ref, :process, self(), :normal})
    Process.sleep(50)
    state = :sys.get_state(pid)
    state_read_ms = System.monotonic_time(:millisecond)

    refute Map.has_key?(state.running, issue_id)
    assert MapSet.member?(state.completed, issue_id)
    assert %{attempt: 1, due_at_ms: due_at_ms} = state.retry_attempts[issue_id]
    assert is_integer(due_at_ms)
    assert_due_after_window(due_at_ms, down_started_ms, state_read_ms, 1_000)
  end
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    assert_due_after_window(due_at_ms, down_started_ms, state_read_ms, 40_000)
  end
  ...
  test "first abnormal worker exit waits before retrying" do
    ...
    assert_due_after_window(due_at_ms, down_started_ms, state_read_ms, 10_000)
  end
  ...
  defp assert_due_after_window(due_at_ms, window_started_ms, window_ended_ms, delay_ms) do
    assert due_at_ms >= window_started_ms + delay_ms
    assert due_at_ms <= window_ended_ms + delay_ms
  end
