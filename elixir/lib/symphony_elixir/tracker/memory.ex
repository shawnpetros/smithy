defmodule SymphonyElixir.Tracker.Memory do
  ...
  def fetch_issues_with_labels(label_names) when is_list(label_names) do
    wanted = MapSet.new(label_names)

    {:ok,
     Enum.filter(issue_entries(), fn %Issue{labels: labels} ->
       labels
       |> List.wrap()
  ...
  def add_label(issue, label_name) do
    send_event({:memory_tracker_label_add, issue, label_name})
    :ok
  end

  @spec remove_label(term(), String.t()) :: :ok | {:error, term()}
  def remove_label(issue, label_name) do
    send_event({:memory_tracker_label_remove, issue, label_name})
    :ok
  end
